...
SAI-1: PRECIP_REGION is NOT currently modelled in this module.
Regional precipitation downscaling (PRECIP_REGION) is available only
through the SAI g6 emulator (mod_sai.py DPRECIP_REGION_SAI).  A
standalone baseline PRECIP_REGION variable would require regional
precipitation coefficients analogous to alpha_temp/beta_temp, which
are not yet available in the calibration data.  This is a known
limitation; precipitation-dependent damage functions should use the
SAI g6 pathway or be added here when suitable calibration data becomes
available.
"""
...
def define_eqs(m, sets, params, cfg, v):
    ...
    return [eq_temp_region, eq_temp_region_dam]
